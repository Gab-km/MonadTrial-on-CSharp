...
namespace MonadTrialTest
{
    public class MaybeIntTest
    {
        ...
        [Test]
        public void doMaybeの引数に1とminus8を与えるとJustMinus7が返ってくること()
        {
            var result = MaybeIntSample.doMaybe("1", "-8");
            Assert.That(result.GetType(), Is.EqualTo(typeof(Just<int>)));
            Assert.That(Maybe<int>.FromJust(result), Is.EqualTo(-7));
        }

        [Test]
        public void doMaybeThisの引数に1と2を与えるとJust3が返ってくること()
        {
            var result = MaybeIntSample.doMaybeThis("1", "2");
            Assert.That(result.GetType(), Is.EqualTo(typeof(Just<int>)));
            Assert.That(Maybe<int>.FromJust(result), Is.EqualTo(3));
        }

        [Test]
        public void doMaybeThisの引数に3と4を与えるとJust7が返ってくること()
        {
            var result = MaybeIntSample.doMaybeThis("3", "4");
            Assert.That(result.GetType(), Is.EqualTo(typeof(Just<int>)));
            Assert.That(Maybe<int>.FromJust(result), Is.EqualTo(7));
        }

        [Test]
        public void doMaybeThisの引数にAと5を与えるとNothingが返ってくること()
        {
            var result = MaybeIntSample.doMaybeThis("A", "5");
            Assert.That(result.GetType(), Is.EqualTo(typeof(Nothing<int>)));
        }

        [Test]
        public void doMaybeThisの引数に6とBを与えるとNothingが返ってくること()
        {
            var result = MaybeIntSample.doMaybeThis("6", "B");
            Assert.That(result.GetType(), Is.EqualTo(typeof(Nothing<int>)));
        }
    }
}
